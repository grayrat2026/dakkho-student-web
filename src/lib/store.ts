...
export type Page =
  ...
  // Help sub-pages
  | 'faq' | 'contact-support' | 'email-us' | 'support-chat' | 'report-issue' | 'terms-of-service' | 'privacy-policy' | 'refund-policy'
  // Exam pages
  ...
  | 'terms-public' | 'privacy-public' | 'rules-public' | 'refund-public' | 'support-public'
  // Error pages
  | 'error-404' | 'error-500';
...
const pageToPath: Record<string, string> = {
  ...
  'error-500': '/error/500',
  // Support pages
  'community-rules': '/help/community-rules',
  'support-wizard': '/support',
  'ticket-detail': '/support/ticket',
  'support-chat': '/help/chat',
  // Public pages (no auth required)
  'terms-public': '/terms',
...
// Explicit overrides for conflicting paths (public pages take priority for these URLs)
pathToPage['/support'] = 'support-wizard';

// ============ PUBLIC PAGES ============
// Pages that can be accessed without authentication and render without AppShell
...
const loadPendingVerification = (): { isPendingVerification: boolean; pendingUserData: { token: string; userId: string; user: User } | null } => {
  ...
  try {
    ...
    if (stored) {
      const parsed = JSON.parse(stored);
      if (parsed.expiresAt && Date.now() < parsed.expiresAt) {
        return { isPendingVerification: true, pendingUserData: parsed.pendingUserData };
      }
      localStorage.removeItem('dakkho-pending-verification');
    }
  } catch {}
  return { isPendingVerification: false, pendingUserData: null };
};

const savePendingVerification = (data: { token: string; userId: string; user: User } | null) => {
  if (typeof window === 'undefined') return;
  try {
    if (data) {
      const expiresAt = Date.now() + 30 * 60 * 1000; // 30 min to verify OTP
      localStorage.setItem('dakkho-pending-verification', JSON.stringify({ pendingUserData: data, expiresAt }));
    } else {
      localStorage.removeItem('dakkho-pending-verification');
    }
  } catch {}
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  isAuthenticated: initialAuth.isAuthenticated && !initialPending.isPendingVerification,
  isLoading: false,
  isPendingVerification: initialPending.isPendingVerification,
  pendingUserData: initialPending.pendingUserData,
  ...
  logout: async () => {
    ...
    clearAuthToken();
    set({ user: null, isAuthenticated: false, isPendingVerification: false, pendingUserData: null });
  ...
  setPendingVerification: (pending: boolean) => {
    set({ isPendingVerification: pending });
  },
  refreshUser: async () => {
    try {
      ...
      if (res.user) {
        // Preserve existing avatarUrl from store if API doesn't return it
