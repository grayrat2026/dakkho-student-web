...
const apiPost = <T>(path: string, body: unknown) => api.post<T>(path, body);
...
export const authApi = {
  signup: (data: { fullName: string; email: string; password: string; instituteId?: number; technology?: string; termsAcceptances?: Array<{ terms_version_id: number; type: string }> }) =>
  ...
  verifyOTP: (data: { email: string; otp: string; tempToken?: string }) =>
    api.post<{ success: boolean; message: string; token?: string; userId?: string; user?: any }>('/api/auth/verify-otp', data),
...
export const paymentApi = {
  ...
  // PipraPay — create payment charge
  create: (data: { packageId: number; couponCode?: string }) =>
    api.post<{ pp_url: string; payment_id: number; amount: number; originalAmount: number; discountApplied: number; couponApplied: number }>('/api/payment/create', data),

  // PipraPay — verify payment
  verify: (data: { pp_id: string }) =>
    api.post<{ status: string; amount: number; gateway: string; transaction_id: string; enrolled_course_id?: string; message?: string }>('/api/payment/verify', data),
};
...
export const studentProfileApi = {
  ...
  uploadSupportAttachment: async (file: File): Promise<{ success: boolean; url: string; key: string }> => {
    const formData = new FormData();
    formData.append('file', file);
    const response = await fetch(`${API_BASE}/api/student/upload-support-attachment`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${getToken()}` },
      body: formData,
    });
...
export const termsApi = {
  getActive: (type?: string) => {
    ...
    return apiGet<{ success: boolean; terms: TermsContent[] }>(`/api/terms/active${params}`);
  },
  getStatus: () =>
    apiGet<{ success: boolean; terms: TermsContent[]; all_accepted: boolean }>('/api/terms/status'),
  accept: (termsVersionIds: number[]) =>
    apiPost<{ success: boolean; message: string }>('/api/terms/accept', { termsVersionIds }),
};
...
export const supportApi = {
  createTicket: (data: {
    ...
    detected_issue?: any;
    user_id?: string;
  }) => apiPost<{ success: boolean; ticket_id: string; message: string; error?: string }>('/api/support/tickets', data),

  lookupTickets: (email: string) =>
    apiGet<{ success: boolean; tickets: any[] }>(`/api/support/tickets/lookup?email=${encodeURIComponent(email)}`),

  getMyTickets: () =>
    apiGet<{ success: boolean; tickets: any[] }>('/api/support/tickets/mine'),

  getTicket: (ticketId: string) =>
    apiGet<{ success: boolean; ticket: any; messages: any[] }>(`/api/support/tickets/${ticketId}`),

  addMessage: (ticketId: string, data: { message: string; sender_type?: string; sender_id?: string; sender_name?: string; attachments?: string[] }) =>
    apiPost<{ success: boolean; message: string }>(`/api/support/tickets/${ticketId}/messages`, data),

  sendEmail: (data: { name: string; email: string; subject: string; message: string }) =>
    apiPost<{ success: boolean; message: string }>('/api/support/email', data),
};
...
export const reviewsApi = {
  getInstructorReviews: (instructorId: string, page?: number, limit?: number) => {
    const params = new URLSearchParams();
    if (page) params.set('page', String(page));
    if (limit) params.set('limit', String(limit));
    const qs = params.toString() ? `?${params.toString()}` : '';
    return apiGet<{ success: boolean; reviews: any[]; stats: any; page: number; limit: number }>(`/api/instructors/${instructorId}/reviews${qs}`);
  },

  createReview: (instructorId: string, data: { rating: number; review?: string; course_id?: string; course_name?: string; is_anonymous?: boolean }) =>
    apiPost<{ success: boolean; message: string }>(`/api/instructors/${instructorId}/reviews`, data),

  markHelpful: (reviewId: string) =>
    apiPut<{ success: boolean }>(`/api/reviews/${reviewId}/helpful`, {}),
  markUnhelpful: (reviewId: string) =>
    apiPut<{ success: boolean }>(`/api/reviews/${reviewId}/unhelpful`, {}),
};
...
export const scheduleApi = {
  getInstructorSchedule: (instructorId: string) =>
    apiGet<{ success: boolean; schedules: any[] }>(`/api/instructors/${instructorId}/schedule`),
};
