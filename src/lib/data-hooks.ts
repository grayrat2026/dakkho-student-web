...
export function useInstructorCourses(instructorId: string | null) {
  const [data, setData] = useState<Course[]>([]);
  const [subjectsData, setSubjectsData] = useState<{ courseId: string; courseTitle: string; subjectName: string }[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchData = useCallback(async () => {
    if (!instructorId) { setData([]); setSubjectsData([]); setLoading(false); return; }
    setLoading(true);
    setError(null);
    try {
      // Fetch all courses and filter by instructor — both direct and through subjects
      ...
      const allCourses = mapApiCourses(rawCourses);

      // Filter courses that have this instructor via direct assignment or through subjects
      const matchedCourseIds = new Set<string>();
      const matchedSubjects: { courseId: string; courseTitle: string; subjectName: string }[] = [];
      ...
      for (let i = 0; i < rawCourses.length; i++) {
        ...
        // Direct assignment
        if (raw.instructor_id === instructorId || raw.instructorId === instructorId || course.instructorId === instructorId) {
          matchedCourseIds.add(course.id);
        }

        // Through subjects — if the API returns subjects embedded on the course
        if (Array.isArray(raw.subjects)) {
          for (const s of raw.subjects) {
            if (s.instructor_id === instructorId || s.instructorId === instructorId) {
              matchedCourseIds.add(course.id);
              matchedSubjects.push({
                courseId: course.id,
                courseTitle: course.title,
                subjectName: s.name || s.title || 'Unnamed Subject',
              });
            }
          }
        }
      ...
      setData(allCourses.filter(c => matchedCourseIds.has(c.id)));
      setSubjectsData(matchedSubjects);
    } catch (err: any) {
  ...
  return { data, subjectsData, loading, error, refetch: fetchData };
}
