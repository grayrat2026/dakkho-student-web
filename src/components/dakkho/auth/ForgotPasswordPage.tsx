...
export function ForgotPasswordPage() {
  ...
  const handleOTPComplete = (otp: string) => {
    // Store the OTP value — actual verification happens on form submit
    setOtpValue(otp);
    setOtpError('');
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setIsResetting(true);
    try {
      const result = await authApi.resetPassword({ email, otp: otpValue, newPassword });
      if (result.success) {
        setStep('success');
      } else {
      ...
      setPasswordError(err?.error || err?.message || 'Failed to reset password. Please try again.');
    } finally {
              ...
              >
                ...
                <OTPInput
                  ...
                  onResend={() => {
                    ...
                    setOtpError('');
                    setOtpValue('');
                    // Resend OTP via forgot-password endpoint
                ...
                <GradientButton type="submit" loading={isResetting} disabled={isResetting} className="w-full" size="lg">
                  Reset Password
                </GradientButton>
              </motion.form>
            )}
