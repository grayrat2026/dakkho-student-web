  ...
  return (
    <div className="pb-20 lg:pb-0">
      ...
      <GlassCard className="overflow-hidden mb-6">
        <div className={`relative aspect-video md:aspect-[21/9] ${course.thumbnailUrl && !thumbnailError ? '' : 'bg-gradient-to-br ' + colorClass}`}>
          ...
          >
            {activeTab === 'overview' && (
              <div className="space-y-6">
                ...
                {/* Subjects & Instructors Section */}
                {course.subjects && course.subjects.length > 0 && (
                  <div className="mt-6">
                    <h2 className="text-xl font-bold mb-4">Subjects & Instructors</h2>
                    <div className="grid gap-3">
                      {course.subjects.map((subject: any, index: number) => (
                        <div
                          key={subject.id || index}
                          className="flex items-center gap-4 p-4 rounded-xl bg-card border border-border hover:border-sky-500/30 transition-colors cursor-pointer"
                          onClick={() => navigate('instructor-profile', { instructorId: subject.instructor_id })}
                        >
                          <div className="flex-shrink-0">
                            {subject.instructor_avatar ? (
                              <img
                                src={subject.instructor_avatar}
                                alt={subject.instructor_name}
                            ...
                            <div className={`w-12 h-12 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white font-bold text-lg ${subject.instructor_avatar ? 'hidden' : ''}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {subject.subject_name_bn && (
                              <p className="text-sm text-muted-foreground">{subject.subject_name_bn}</p>
                            )}
                            <p className="text-sm text-sky-500 mt-0.5">{subject.instructor_name}</p>
                            {subject.instructor_specialization && (
                              <p className="text-xs text-muted-foreground">{subject.instructor_specialization}</p>
                            )}
                          </div>
                ...
                {instructor && (
                  <GlassCard className="p-6">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      >
                        {instructor.avatarUrl && !instructorAvatarError ? (
                          <img
