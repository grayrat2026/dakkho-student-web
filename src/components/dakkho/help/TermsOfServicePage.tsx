...
export function TermsOfServicePage() {
  ...
  const [apiContent, setApiContent] = useState<{title: string; content: string; title_bn?: string; content_bn?: string} | null>(null);
  const [loadingApi, setLoadingApi] = useState(true);

  useEffect(() => {
    async function fetchContent() {
      try {
        const result = await termsApi.getActive('terms_of_service');
  ...
  const content: Record<string, { title: string; body: string[] }> = {
    ...
    accounts: {
      ...
      body: [
        ...
        'You must not share your account credentials with any third party. You are solely responsible for all activities that occur under your account. If you suspect unauthorized access to your account, you must notify us immediately at security@dakkho.pro.bd.',
    ...
    modifications: {
      ...
      body: [
        ...
        'These Terms were last updated on January 15, 2025. For questions about these Terms, please contact us at legal@dakkho.pro.bd.',
      ],
    },
  };
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <GlassCard className="p-6">
            <div className="prose prose-sm dark:prose-invert max-w-none">
              ...
              <div dangerouslySetInnerHTML={{ __html: markdownToHtml(apiContent.content) }} />
            </div>
          </GlassCard>
        </motion.div>
      ) : (
