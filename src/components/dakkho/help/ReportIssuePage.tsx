...
export function ReportIssuePage() {
  ...
  const handleAttach = () => {
    if (attachmentPreviews.length < 5) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const remaining = 5 - attachmentPreviews.length;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await supportApi.createTicket({
        ...
        priority: severity,
        subject: title,
        description: `**Steps to Reproduce:**\n${steps.filter(s => s.trim()).map((s, i) => `${i + 1}. ${s}`).join('\n')}\n\n**Expected Behavior:**\n${expectedBehavior}\n\n**Actual Behavior:**\n${actualBehavior}\n\n**Environment:** Browser: ${browser || 'N/A'}, Device: ${device || 'N/A'}`,
      ...
      if (result.success) {
        ...
        setSubmitError(result.error || 'Failed to submit bug report');
      }
    } catch (err: any) {
    ...
    return (
      <div className="pb-20 lg:pb-0">
        ...
        <GlassCard className="p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-1">Report ID: <span className="font-bold text-sky-500">{ticketId}</span></p>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <GlassCard className="p-5">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground block mb-1.5">Screenshots ({attachmentPreviews.length}/5)</label>
              <input
                ref={fileInputRef}
                type="file"
                multiple
              ...
              {attachmentPreviews.length > 0 && (
                <div className="mt-2 space-y-1.5">
                  {attachmentPreviews.map((file, i) => (
                    <motion.div key={`${file}-${i}`} className="flex items-center justify-between p-2 rounded-lg bg-muted/20" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
