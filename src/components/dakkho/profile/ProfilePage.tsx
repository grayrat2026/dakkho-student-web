...
// formatTimeAgo is now imported from @/lib/mock-data
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        // Update avatarUrl in store if available from profile
        if (res.profile?.avatarUrl && user && !user.avatarUrl) {
          setUser({ ...user, avatarUrl: res.profile.avatarUrl });
        }
      } catch (err) {
            ...
            {activities.map((activity, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-muted-foreground flex-shrink-0">{formatTimeAgo(activity.createdAt)}</span>
                </motion.div>
              );
            })}
          </GlassCard>
