...
import { usePipraPay } from '@/hooks/use-piprapay';
...
export function SubscriptionPage() {
  ...
  // PipraPay hook
  const { handleBuy: pipraPayBuy, loading: pipraPayLoading, error: pipraPayError } = usePipraPay();

  // Determine if PipraPay is the active gateway
  const isPipraPayActive = paymentConfigs.some((c) => c.gateway?.toLowerCase() === 'piprapay' && c.is_active === 1);
                  ...
                  {isPipraPayActive && (
                    <GlassCard className="p-4 mb-4">
                      <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                        ...
                        Quick Pay with PipraPay
                      </h3>
                      <p className="text-xs text-muted-foreground mb-3">
                        Pay instantly using bKash, Nagad, or other methods via PipraPay gateway. No manual verification needed.
                      </p>
                      {pipraPayError && (
                        <motion.div
                          initial={{ opacity: 0, y: -5 }}
                          animate={{ opacity: 1, y: 0 }}
                          className="mb-3 flex items-start gap-2 p-3 rounded-xl bg-red-50/50 dark:bg-red-900/20 border border-red-200/50 dark:border-red-700/30"
                        >
                          <AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5" />
                          <p className="text-xs text-red-600 dark:text-red-400">{pipraPayError}</p>
                        </motion.div>
                      )}
                      <GradientButton
                        className="w-full"
                        onClick={() => pipraPayBuy(selectedPackage.id, coupon.valid && coupon.code ? coupon.code : undefined)}
                        disabled={pipraPayLoading}
                        loading={pipraPayLoading}
                        variant="success"
                      >
                        <Zap className="w-4 h-4" />
                        {pipraPayLoading ? 'Redirecting to Payment...' : 'Pay with PipraPay'}
                      </GradientButton>
                      ...
                      <div className="relative my-4">
                        ...
                        <div className="relative flex justify-center text-xs">
                          <span className="px-2 bg-white/70 dark:bg-slate-900/70 text-muted-foreground">or pay manually</span>
                        </div>
                      </div>
                    </GlassCard>
                  )}
