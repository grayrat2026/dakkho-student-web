...
function Toggle({ enabled, onToggle, color = 'sky', disabled = false }: { enabled: boolean; onToggle: () => void; color?: string; disabled?: boolean }) {
  const bgColor = color === 'emerald' ? 'bg-emerald-500' : color === 'amber' ? 'bg-amber-500' : 'bg-sky-500';
  return (
    <motion.button
      className={`w-11 h-6 rounded-full p-0.5 transition-colors ${enabled ? bgColor : 'bg-muted'} ${disabled ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}
...
const DEFAULT_SETTINGS: NotifSetting[] = [
  ...
  { id: 'grades', label: 'Grades & Results', description: 'Exam results and grade postings', icon: Award, push: true, email: true, sms: false },
  { id: 'schedule', label: 'Schedule Changes', description: 'Class cancellations and room changes', icon: Calendar, push: true, email: false, sms: false },
  ...
  { id: 'alerts', label: 'System Alerts', description: 'Maintenance and security notifications', icon: AlertTriangle, push: true, email: true, sms: false },
];

function mapPrefsToSettings(prefs: NotificationPreferences): NotifSetting[] {
  return [
    { ...DEFAULT_SETTINGS[0], push: prefs.courseUpdates?.push ?? DEFAULT_SETTINGS[0].push, email: prefs.courseUpdates?.email ?? DEFAULT_SETTINGS[0].email, sms: false },
    { ...DEFAULT_SETTINGS[1], push: prefs.grades?.push ?? DEFAULT_SETTINGS[1].push, email: prefs.grades?.email ?? DEFAULT_SETTINGS[1].email, sms: prefs.smsEnabled ?? false },
    { ...DEFAULT_SETTINGS[2], push: prefs.schedule?.push ?? DEFAULT_SETTINGS[2].push, email: prefs.schedule?.email ?? DEFAULT_SETTINGS[2].email, sms: false },
    { ...DEFAULT_SETTINGS[3], push: prefs.payment?.push ?? DEFAULT_SETTINGS[3].push, email: prefs.payment?.email ?? DEFAULT_SETTINGS[3].email, sms: false },
    { ...DEFAULT_SETTINGS[4], push: prefs.promotions?.push ?? DEFAULT_SETTINGS[4].push, email: prefs.promotions?.email ?? DEFAULT_SETTINGS[4].email, sms: false },
    { ...DEFAULT_SETTINGS[5], push: prefs.social?.push ?? DEFAULT_SETTINGS[5].push, email: prefs.social?.email ?? DEFAULT_SETTINGS[5].email, sms: false },
    { ...DEFAULT_SETTINGS[6], push: prefs.system?.push ?? DEFAULT_SETTINGS[6].push, email: prefs.system?.email ?? DEFAULT_SETTINGS[6].email, sms: prefs.smsEnabled ?? false },
  ];
}
...
export function NotificationSettingsPage() {
  ...
  const [adminSmsEnabled, setAdminSmsEnabled] = useState(false);
  ...
  const loadSettings = useCallback(async () => {
    setLoading(true);
    try {
      const result = await studentSettingsApi.get() as any;
      const prefs = result.preferences as NotificationPreferences;
      if (prefs) {
      ...
      // Check admin SMS control
      if (typeof result.adminSmsEnabled === 'boolean') {
        setAdminSmsEnabled(result.adminSmsEnabled);
      }
    } catch {
  ...
  const handleSave = async () => {
    ...
    setSaveError(null);
    try {
      // If admin SMS is disabled, force smsEnabled to false
      const effectiveSMS = adminSmsEnabled ? masterSMS : false;
      const preferences = mapSettingsToPrefs(settings, masterPush, masterEmail, effectiveSMS, quietHours ? quietStart : '22:00', quietHours ? quietEnd : '08:00');
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {adminSmsEnabled
                          ...
                          : 'Disabled by admin — contact support to enable'}
                      </p>
                    </div>
                  </div>
                  <Toggle
                    enabled={adminSmsEnabled && masterSMS}
                    onToggle={() => setMasterSMS(!masterSMS)}
                    color="amber"
                    disabled={!adminSmsEnabled}
                  />
                </div>
                {!adminSmsEnabled && (
                  <motion.div
                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-900/10 border border-amber-200/50 dark:border-amber-800/30"
                    initial={{ opacity: 0, height: 0 }}
                    animate={{ opacity: 1, height: 'auto' }}
                  >
                    <Lock className="w-3.5 h-3.5 text-amber-500 flex-shrink-0" />
                    <p className="text-[11px] text-amber-600 dark:text-amber-400">SMS notifications are currently disabled by the administrator. Please contact support if you need this feature.</p>
                  </motion.div>
                )}
              </div>
            </GlassCard>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            <GlassCard className="p-5">
              ...
              <div className="space-y-1">
                {/* Column headers */}
                <div className={`grid gap-2 items-center pb-2 border-b border-white/20 dark:border-white/5 ${adminSmsEnabled ? 'grid-cols-[1fr_44px_44px_44px]' : 'grid-cols-[1fr_44px_44px]'}`}>
                  ...
                  {adminSmsEnabled && <span className="text-xs font-bold text-muted-foreground text-center">SMS</span>}
                </div>
                {settings.map((s, i) => (
                  <motion.div
                    key={s.id}
                    className={`grid gap-2 items-center py-3 border-b border-white/10 dark:border-white/5 last:border-0 ${adminSmsEnabled ? 'grid-cols-[1fr_44px_44px_44px]' : 'grid-cols-[1fr_44px_44px]'}`}
                  ...
                  >
                    ...
                    <div className="flex justify-center">
                      <Toggle
                        enabled={s.push && masterPush}
                        onToggle={() => toggleChannel(s.id, 'push')}
                        disabled={!masterPush}
                      />
                    </div>
                    <div className="flex justify-center">
                      <Toggle
                        enabled={s.email && masterEmail}
                        onToggle={() => toggleChannel(s.id, 'email')}
                        color="emerald"
                    ...
                    {adminSmsEnabled && (
                      <div className="flex justify-center">
                        <Toggle
                          enabled={s.sms && masterSMS}
                          onToggle={() => toggleChannel(s.id, 'sms')}
                          color="amber"
                          disabled={!masterSMS}
                        />
                      </div>
