...
export function mapApiCourse(raw: any): Course {
  return {
    ...
    tags: Array.isArray(raw.tags) ? raw.tags : (typeof raw.tags === 'string' && raw.tags ? raw.tags.split(',').map((t: string) => t.trim()).filter(Boolean) : []),
...
// formatTimeAgo is now imported from @/lib/mock-data (single source of truth)
