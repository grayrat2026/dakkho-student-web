...
export function CourseCard({ course, showProgress = false, progress = 0, index = 0, instructors: instructorsProp, categories: categoriesProp }: CourseCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Thumbnail */}
        <div className={`relative aspect-video overflow-hidden ${course.thumbnailUrl ? '' : `bg-gradient-to-br ${colorClass}`}`}>
          {course.thumbnailUrl ? (
            <img
              ...
              onError={(e) => {
                ...
                if (parent) {
                  parent.classList.add('bg-gradient-to-br', colorClass);
          ...
          <div className={`absolute inset-0 flex items-center justify-center thumbnail-fallback ${course.thumbnailUrl ? 'hidden' : ''}`}>
